...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg) {
  ...
  // reboot_cmd = ((double)msg.channels[7] - 1000.0) / 1000.0; 取消reboot
  // ekf2功能
...
void RC_Data_t::check_validity() {
  if (mode >= -1.1 && mode <= 1.1) {
    ...
    ROS_ERROR("RC data validity check fail. mode=%f", mode);
  }
}

bool RC_Data_t::check_centered() {
  bool centered = abs(ch[0]) < 1e-5 && abs(ch[0]) < 1e-5 && abs(ch[0]) < 1e-5 &&
                  abs(ch[0]) < 1e-5;
  return centered;
}
