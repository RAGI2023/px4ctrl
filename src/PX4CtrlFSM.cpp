  ...
  switch (state) {
    case MANUAL_CTRL: {
      if (rc_data.enter_hover_mode)  // Try to jump to AUTO_HOVER
      {
        ...
        if (cmd_is_received(now_time)) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_HOVER(L2). You are sending commands "
        ...
        if (odom_data.v.norm() > 3.0) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_HOVER(L2). Odom_Vel=%fm/s, which seems "
              "that the locolization module goes wrong!",
              odom_data.v.norm());
          break;
        }
        ...
        ROS_INFO(
            "\033[32m[px4ctrl] MANUAL_CTRL(L1) --> AUTO_HOVER(L2)\033[32m");
                                                            ...
                                                            // AUTO_TAKEOFF
      {
        ...
        if (cmd_is_received(now_time)) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_TAKEOFF. You are sending commands before "
              "toggling into AUTO_TAKEOFF, which is not allowed. Stop sending "
              "commands now!");
          break;
        }
        if (odom_data.v.norm() > 0.1) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_TAKEOFF. Odom_Vel=%fm/s, non-static "
        ...
        if (!get_landed()) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_TAKEOFF. land detector says that the "
          ...
          if (!rc_data.is_hover_mode || !rc_data.is_command_mode ||
            ...
            while (ros::ok()) {
              ...
              if (rc_data.is_hover_mode && rc_data.is_command_mode &&
                  rc_data.check_centered()) {
                ROS_INFO(
                    "\033[32m[px4ctrl] OK, you can takeoff again.\033[32m");
                break;
              }
            }
            break;
    ...
    case AUTO_TAKEOFF: {
      if ((now_time - takeoff_land.toggle_takeoff_land_time).toSec() <
          AutoTakeoffLand_t::MOTORS_SPEEDUP_TIME)  // Wait for several seconds
                                                   // to warn prople.
      {
    ...
    case AUTO_LAND: {
      if (!rc_data.is_hover_mode || !odom_is_received(now_time)) {
        ...
        if (print_once_flag) {
          ROS_INFO(
              "\033[32m[px4ctrl] Wait for abount 10s to let the drone "
              "arm.\033[32m");
                ...
                LANDED_STATE_ON_GROUND)  // PX4 allows disarm after this
        {
          ...
          if (now_time.toSec() - last_trial_time > 1.0) {
            if (toggle_arm_disarm(false))  // disarm
            {
              ...
              ROS_INFO(
                  "\033[32m[px4ctrl] AUTO_LAND --> MANUAL_CTRL(L1)\033[32m");
            }
